<v-app-bar app fixed light clipped-left clipped-right 
    class="app-navbar v-sheet v-sheet--tile theme--light v-toolbar v-app-bar v-app-bar--clipped v-app-bar--fixed v-app-bar--is-scrolled white"
    height="60" 
    :color="navbarColor" 
    :dark="navbarDark" 
    :flat="navbarFlat">
    ...
    <div class="v-toolbar__items hidden-sm-and-down">
        <?php foreach ($menu_items as $menu_item) {
            echo sprintf(
                ...
                $menu_item['url'],
                $menu_item['active'] ? 'success--text' : 'success--text',
                $menu_item['title']
            );
        } ?>
    </div>
