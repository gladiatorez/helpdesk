    ...
    <v-container fluid class="pl-5 pr-5 pb-5">
        <v-row>
            <v-col cols="12" lg="9" md="7">
                    ...
                    <v-tabs centered>
                        ...
                        <v-tab-item>
                            <manage-ticket
                                ...
                                @rejected-success="onApproveSuccess"
                                @remove-staff-success="onApproveSuccess"
                            ></manage-ticket>
                        </v-tab-item>
                        ...
                        <v-tab-item>
                            <ticket-detail-note :notes="item.notes" />
                        </v-tab-item>
                            ...
                            <v-row class="mx-0">
                                <v-col cols="12" md="6">
                                    <part-list
                                        ...
                                        :hide-form="item.flag === 'CLOSED' || item.flag === 'FINISHED'"
                                        @delete="fetchItem"
                                    />
                                </v-col>
                ...
                <v-card>
                    <template v-if="item.flag !== 'CLOSED'  && item.flag !== 'CANCEL' && $can('manage','tickets')">
            ...
            <v-col cols="12" lg="3" md="5">
                <ticket-detail-timeline :logs="item.logs" />
            </v-col>
        </v-row>
    </v-container>
</template>
...
import PartList from "./includes/PartList.vue";
import PartPhotoList from "./includes/PartPhotoList.vue";
...
export default {
    ...
    methods: {
        ...
        fetchItem() {
            ...
            this.loading = true;
            this.$axios
                .get("tickets/item", { params: { id: this.item.id } })
                .then(response => {
                    const { data } = response;
                    const row = data.data;
