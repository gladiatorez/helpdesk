<template>
    <v-dialog v-model="opened" max-width="500px" persistent scrollable>
        <v-card :disabled="loading" :loading="loading">
            <v-card-title>
                <span class="font-weight-black">Change flag</span>
            </v-card-title>
            <v-card-text>
                <v-select
                    v-model="flagRow"
...
export default {
    ...
    computed: {
        ...
        flagfOptions() {
            ...
            if (this.$can('close_ticket','queues/assignment')) {
    ...
    methods: {
        ...
        addCloseAction() {
            ...
            this.$axios.post('queues/assignment/change_flag', item).then((response) => {
                const { data } = response;
