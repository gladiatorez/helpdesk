		...
		<v-list dense>
			<v-list-item>
				<v-list-item-icon style="width: 150px">
					<span class="subtitle-2 font-weight-regular grey--text text--darken-1">{{$t('profile::full_name')}}</span>
				</v-list-item-icon>
				<v-list-item-content>
			...
			<v-list-item>
				<v-list-item-icon style="width: 150px">
					<span class="subtitle-2 font-weight-regular grey--text text--darken-1">{{$t('profile::phone')}}</span>
				</v-list-item-icon>
				<v-list-item-content>
...
export default {
	...
	methods: {
		...
		saveChangeAction() {
			...
			this.$axios.post('profile/savegeneral', item)
				.then((response) => {
					const { data } = response;
