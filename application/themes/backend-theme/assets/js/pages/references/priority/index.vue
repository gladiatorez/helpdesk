        ...
        <v-card flat>
            <!-- <v-card-title>
                <v-btn
                    ...
                    color="primary"
                    class="white--text"
                    :to="{ name: 'references.priority.create' }">
    ...
    data() {
        return {
            headers: [
                ...
                // },
                {
                    text: this.$t("references::priority:name"),
                    value: "name"
                },
                {
                    text: this.$t("references::priority:order"),
                    value: "ord",
                    width: "120px"
                },
                {
    ...
    methods: {
        ...
        removeAction(item) {
            ...
            if (confirm(confirmText)) {
                ...
                const that = this;
                this.$axios
                    .get("references/priority/remove/", {
                        params: { id: item.id }
                    })
                    .then(response => {
                        const { data } = response;
