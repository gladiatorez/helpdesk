...
class The_auth_frontend
{
	...
	public function __construct()
	{
        ...
        $this->_ci->load->config('auth/the_auth_frontend', TRUE);
        $this->_ci->load->helper('cookie');
        $this->_ci->load->helper('auth/the_auth_frontend');
    ...
    {
        $this->_config = $this->_ci->config->config['the_auth_frontend'];
        ...
        $this->_storeSalt = (bool) Setting::get('front_auth_store_salt', 'frontend_auth');
        $this->_storeSaltLength = Setting::get('front_auth_store_salt_length', 'frontend_auth');
        $this->_loginIdentity = Setting::get('front_auth_login_identity', 'frontend_auth');
        $this->_loginIdentityCookie = Setting::get('front_auth_identity_cookie', 'frontend_auth');
        $this->_maxLoginAttempts = Setting::get('front_auth_max_login_attempts', 'frontend_auth');
        $this->_rememberMeExpire = Setting::get('front_auth_remember_me_expire', 'frontend_auth');
        $this->_rememberMeCookieName = Setting::get('front_auth_remember_me_cookie', 'frontend_auth');
        $this->_recheckTimer = Setting::get('front_auth_recheck_timer', 'frontend_auth');
        $this->_extendOnLogin = (bool) Setting::get('front_auth_extend_on_login', 'frontend_auth');
        if ((int) Setting::get('front_auth_min_password_length', 'frontend_auth') >= 5) {
            $this->_minPasswordLength = (int) Setting::get('front_auth_min_password_length', 'frontend_auth');
        }
        $this->_maxPasswordLength = (int) Setting::get('front_auth_max_password_length', 'frontend_auth');
        $this->_forgottenPasswordExpired = (int)Setting::get('front_auth_forgot_password_expired', 'frontend_auth');
    }
    ...
    {
        $userId = isset($userId) ? $userId : $this->_ci->session->userdata('front_user_id');
        ...
        $user = $this->_ci->{$this->_userModel}
            ...
            ->fields('id,ip_address,username,email,activation_code,forgotten_password_code,forgotten_password_time,remember_code,active,lang,group_id,last_login,company_id')
            ->with('informer', ['fields:full_name,phone,photo_file,updated_at,nik,position,company_id,company_branch_id,company_other,department_id,department_other'])
    ...
    {
        $userId = $this->_ci->session->userdata('front_user_id');
        if (!empty($userId)) {
            return $userId;
        }
	...
	public function register($username, $password, $email, $company_id, $groupId = null, $profile = [], $lang = 'id', $active = FALSE)
    {
        ...
        if (!$groupId) {
            $group = $this->_ci->{$this->_groupModel}->getInformerGroup();
            if (!$group) {
        ...
        $register = $this->_ci->{$this->_userModel}->create($user, $profile, 'informer/informer_model');
        if ($register) {
    ...
    {
        $sessionData = [
            'front_identity'              => $user->{$this->_loginIdentity},
            'front_' . $this->_loginIdentity   => $user->{$this->_loginIdentity},
            'front_email'                 => $user->email,
            'front_user_id'               => $user->id,
            'front_old_last_login'        => $user->last_login,
            'front_last_check'            => time()
        ];
    ...
    public function doLogin($identity, $password, $rememberMe = false)
    {
        ...
        $groups = $this->_ci->{$this->_groupModel}->get_all(['send_ticket' => true]);
        if (!$groups) {
    ...
    public function doLogout()
    {
        ...
        $this->_ci->session->unset_userdata(['front_identity', 'front_' . $this->_loginIdentity, 'front_email', 'front_user_id', 'front_old_last_login', 'front_last_check']);
        if (get_cookie($this->_loginIdentityCookie)) {
    ...
    protected function reCheckSession()
    {
        ...
        if ($recheck !== 0)
        {
            $lastLogin = $this->_ci->session->userdata('front_last_check');
            if ($lastLogin + $recheck < time())
            {
                $user = $this->_ci->{$this->_userModel}->get([
                    $this->_loginIdentity => $this->_ci->session->userdata('front_identity'),
                    'active' => '1'
                ]);
                if ($user) {
                    $this->_ci->session->set_userdata('front_last_check', time());
                }
                else {
                    $this->_ci->session->unset_userdata(array('front_'.$this->_loginIdentity, 'front_id', 'front_user_id'));
                }
            }
        }

        return $this->_ci->session->userdata('front_identity')
            && $this->_ci->session->userdata('front_' . $this->_loginIdentity)
            && $this->_ci->session->userdata('front_user_id');
    }
    ...
    public function forgottenPassword($email)
    {
        ...
        $findUser = $this->_ci->{$this->_userModel}->with('informer')
            ->where('group_id', $groupIds)
            ->get(['email' => $email]);
        if (!$findUser) {
            $this->setError('auth::forgot_password_unsuccessful', TRUE);
        ...
        if ($this->_ci->db->affected_rows()) 
        {
            ...
            $data['user']               = $findUser;
            $data['url_forgotten']      = site_url('auth/reset-password');
            $data['url_forgotten_code'] = site_url('auth/reset-password/' . $key);
